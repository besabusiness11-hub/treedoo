...
import { Edit3, ArrowLeft, Calendar, Info, ScrollText } from "lucide-react"
    ...
    <div className="animate-in fade-in duration-500 bg-gray-50 min-h-screen pb-32">
      ...
      <header className="bg-[#1e3a8a] text-white pt-12 pb-8 px-6 rounded-b-[2.5rem] shadow-md">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-xl font-extrabold tracking-tight">Verbali Assemblee</h1>
        </div>
        <p className="text-blue-200 text-sm">Consulta i verbali delle assemblee condominiali.</p>
      </header>

      <div className="px-6 pt-6 space-y-4">
        {data.verbali.length > 0 ? (
          data.verbali.map((doc) => (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="w-16 h-16 bg-violet-50 rounded-2xl flex items-center justify-center mb-4">
              <Edit3 className="w-8 h-8 text-violet-300" />
            </div>
            <h3 className="font-bold text-slate-700 text-lg">Nessun verbale disponibile</h3>
            <p className="text-sm text-slate-500 mt-2 max-w-[280px]">
              I verbali delle assemblee condominiali verranno pubblicati qui dall'amministratore dopo ogni riunione.
            </p>
            <div className="mt-6 flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-2xl p-4 text-left max-w-sm">
              ...
              <p className="text-xs text-amber-800 font-medium">
                Se hai partecipato ad un'assemblea recente di cui non trovi il verbale, contatta l'amministratore tramite la Dashboard.
              </p>
            </div>
          </div>
        )}
